...
ym2413はmode0で演奏後mode1移行で状態引継がれリズム音がなってしまうため瞬時の無音初期化は不可能です。モード変更時に継続演奏したいなら現状と設定状態を持ち演奏時に差があれば音量設定するようにして、ハードの初期化は音量ゼロで、設定値はデフォルト音量で始めます。リズム音源を鳴らすにはレジスタ0x0eの0-5ビットをOFFからONにする必要があります。ONからONではキックされません。音楽データはリズム音モードのチャンネルデータの有無で判定するようにします。ドライバは開始時にリズムのチャンネルデータが有ればリズム音モードフラグをONにしてアップデート処理でリズム音専用の関数を呼び出して処理します。リズム音再生時は音程は必要ないです。PKEYOFF,PWAIT,PVOLUME,PEND,PLOOP,PNEXT,PBREAKの通常動作が動きます。トーンの処理がなく分岐は線形探索的なので別関数にわけリズム音を鳴らします。プログラムサイズ縮小を考えるとスイッチ以降の処理は同じものが使えるので、アセンブラで開発する場合はスイッチの処理をコピーしそれ以降の処理は共有することで容量を削減します。リズム音の音量設定も処理が変わるポイントですが、最初のバージョンでは音量設定に対応しません。音量コントロールと初期化は話が複雑になるので後回しにします。

とにかくまずは、リズム音モードにして鳴らすことを試みます。コンパイラはドラムのデータを元にPDRUMとPDRUMVOLを発行します。リズムデータは普通に処理した後、ヘッダ情報をまとめるところで名前だけ前にもってこようとしてなければNULLで埋めるかんじにします。
